 ...
 *  common.js  |  shared by desktop and mobile, and by
 *  index.html (EN) and hebrew.html (HE)
 * ============================================================
 *
 *  Everything in this file runs on every device. Device-specific
 *  behaviour lives in two sibling files that this file loads
 *  conditionally, right after DOMContentLoaded runs:
 *
 *    desktop.js  →  fetched only on viewports ≥ 1101px
 *                   (card tilt + cursor spotlight — needs a
 *                   fine pointer, so it's meaningless on touch)
 *    mobile.js   →  fetched only on viewports ≤ 1100px
 *                   (the swipeable shows carousel)
 *
 *  Only one of those two files is ever requested — see
 *  loadDeviceScript() at the bottom of this file.
 *
 *  initTestimonialsCarousel() below is deliberately NOT split
 *  even though it adapts its behaviour by viewport width
 *  (cardsPerView() checks window.innerWidth, and the whole
 *  carousel also handles touch swipes). It already lives at the
 *  same live-resize boundary as the CSS split (it recomputes on
 *  window 'resize', not just at load), so pulling it apart into
 *  device-only and mobile-only halves would mean two files
 *  sharing one autoplay timer / drag-state closure — a source of
 *  duplicate-state bugs, not a real separation of concerns. It
 *  stays here as a single, genuinely shared, adaptive component.
 *
 *  Table of Contents
 *  -----------------
 *  1.  Config & Utilities
 ...
 * 14.  Staggered Reveal (grid/list children)
 * 15.  Device Script Loader (desktop.js / mobile.js)
 ...
 * Colors reference the CSS custom properties defined in base.css,
...
/* ============================================================
   7.  SCROLL-REVEAL ANIMATIONS
   ============================================================
   Targets every [data-reveal] wrapper; base.css owns the actual
...
/* ============================================================
   8.  TESTIMONIALS CAROUSEL
   ============================================================
   Genuinely shared/adaptive — see the file header for why this
   isn't split into desktop/mobile halves.
   ============================================================ */
...
/* ============================================================
   ...
   --stagger-index custom property so base.css can cascade
...
/* ============================================================
   15.  DEVICE SCRIPT LOADER  (desktop.js / mobile.js)
   ============================================================
   Fetches exactly one of the two device-specific files, based on
   the same 1101px boundary the CSS split uses, and runs its init
   function once it has loaded. Nothing else in this file depends
   on desktop.js/mobile.js, so this is the only coupling point.
   ============================================================ */
...
function loadDeviceScript() {
    ...
    script.onload = () => {
        if (isDesktop && typeof initCardInteractions === 'function') {
            initCardInteractions();
        } else if (!isDesktop && typeof initShowsCarousel === 'function') {
            initShowsCarousel();
        }
    };
